...
public class MTCSC {
     ...
     */
    public MTCSC(TimeSeries timeseries, double sMax, int t, int n) {
    ...
    public TimeSeries getTimeSeries() {return this.timeseries;}
...
//        ArrayList<Double> kp_modify = kp.getModify();
        DMatrixRMaj pp_modify = prePoint.getValRepaired();
        DMatrixRMaj kp_modify = kp.getValRepaired();
        for (int i = 0; i < this.N; i++) {
            distance += Math.pow(pp_modify.get(i, 0) - kp_modify.get(i, 0), 2);
        }
        return distance;
    }
    ...
    private boolean judgeModify(DMatrixRMaj preVal, DMatrixRMaj maxVal, DMatrixRMaj kpVal,
        ...
        double c1 = 0, c2 = 0;
        for (int i = 0; i < this.N; i++) {
            c1 += Math.pow(preVal.get(i, 0) - kpVal.get(i, 0), 2);
        }
        for (int i = 0; i < this.N; i++) {
            c2 += Math.pow(maxVal.get(i, 0) - kpVal.get(i, 0), 2);
        }
...
//        List<Double> preVal = prePoint.getModify();
        long kpTime = kp.getTimestamp();
//        List<Double> kpVal = kp.getModify();
...
//            ArrayList<Double> N_modify = new ArrayList<Double>();
            DMatrixRMaj N_modify = new DMatrixRMaj(N, 1);
            ...
            if (pre_next_dis > pre_dis) {
                for (int i = 0; i < this.N; i++) {
                    double tmp = (maxVal.get(i, 0) - preVal.get(i, 0)) * (pre_dis / pre_next_dis) + preVal.get(i, 0);
//                    N_modify.add(tmp);
                    N_modify.set(i, tmp);
                }
            } else {
                for (int i = 0; i < this.N; i++) {
                    double tmp = (maxVal.get(i, 0) + preVal.get(i, 0)) / 2;
//                    N_modify.add(tmp);
                    N_modify.set(i, tmp);
                }
            }
//            kp.setModify(N_modify);
            kp.setValRepaired(new DMatrixRMaj(N_modify));
        }
    }
}
