...
import com.MINOR.Utils.AkaneUtils;
import com.MINOR.Utils.AkaneUtils.Triplet;
 ...
 * Akane Heuristic
 * Akane encountered memory overflow issues when using dynamic programming, so we chose the Akane Heuristic as baseline
 */
public class AkaneHeuristic {
    ...
    private UniTimeSeries resultTimeSeries;

    public AkaneHeuristic(UniTimeSeries timeSeries, int k, int[] budgetlist) {
    ...
    public AkaneHeuristic(UniTimeSeries timeSeries, int k, int[] budgetlist, int r) {
        setTimeseries(timeSeries);
        setK(k);
        setBudgetList(budgetlist);
        setR(r);
    ...
    public AkaneHeuristic(UniTimeSeries timeSeries, int k) {
        setTimeseries(timeSeries);
        setK(k);
        setBudgetList(null);
        setR(-1);
    ...
    public UniTimeSeries getResultTimeSeries() {
    ...
    public void setBudgetList(int[] budgetlist) {
...
//    return probability;
    ...
    public static ArrayList<Pair<Integer, String>> getCleanedSymbols(
        ...
        AkaneUtils assist = new AkaneUtils();
...
//        System.out.println("Automatically select budget " + (cnt - existingcnt));
    ...
    /**
     * won't be used in the experiment
     */
    public ArrayList<UniTimeSeries> mainAkaneHeuristic() {
        if (Budgetlist == null) {
...
//    System.out.println(maxBudget);
        ...
        ArrayList<UniTimeSeries> resultSeriesList = new ArrayList<>();
...
//               centroids, n);
            ...
            UniTimeSeries resultSeires = assist.List2TS(cleandata);
    ...
    public RepairResult mainAkaneHeuristicAuto() {
...
//        UniTimeSeries resultSeries = assist.List2TS(cleandata);
        long endTime = System.nanoTime();
        double duration = ((double) (endTime - startTime)) / 1_000_000.0d;
        UniTimeSeries finalResult = assist.applyCleanedData(cleandata, timeseries);
        this.resultTimeSeries = finalResult;
        return new RepairResult(MyMathUtils.calculateRMSE(finalResult), duration, 1);
//        return resultSeries;
    }
}
