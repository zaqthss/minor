...
/**
 * Varying data size on ECG
 */
public class DataSize_ECG {
...
//        int[] sizes = Constants.sizesECG;
        ...
        for (int i = 0; i < caseCnt; i++) {
            for (int seed = 0; seed < repeatTime; seed++) {
                ts = new TimeSeries();
                dataSetName = dt + "_" + et + "_" + variableName + "_" + sizes[i] + '_' + seed + ".csv";
        ...
        MyFileUtils.exportResult(fileName, modelNames, sizes, totalRMSE);
        fileName = basePath + "TIME.csv";
        MyFileUtils.exportResult(fileName, modelNames, sizes, totalTime);
        fileName = basePath + "iterationNum.csv";
        MyFileUtils.exportResult(fileName, modelNames, sizes, totalIterNum);
    }
}
