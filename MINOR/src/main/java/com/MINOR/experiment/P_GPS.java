...
public class P_GPS {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < caseCnt; i++) {
            for (int seed = 0; seed < repeatTime; seed++) {
                ...
                if (flags[modelID]) {
                    ...
                    MINOR_B minorB = new MINOR_B(ts, p, threshold0, maxIterations);
                    result = minorB.run();
                ...
                if (flags[modelID]) {
                    System.out.println("[INFO]running " + modelNames[modelID]);
                    MyFileUtils.csv2MTS(ts, dim, dataFPath);
                    MINOR_U minorU = new MINOR_U(ts, p, threshold0, maxIterations);
                    result = minorU.run();
                    totalRMSE[i][modelID] += result.getRmse();
                    totalTime[i][modelID] += result.getTimeCost();
                    totalIterNum[i][modelID] += result.getIterationNum();
                ...
                if (flags[modelID]) {
                    System.out.println("[INFO]running " + modelNames[modelID]);
                    MyFileUtils.csv2MTS(ts, dim, dataFPath);
                    VARX varx = new VARX(ts, p, threshold0);
                    result = varx.run();
                    totalRMSE[i][modelID] += result.getRmse();
                    totalTime[i][modelID] += result.getTimeCost();
                    totalIterNum[i][modelID] += result.getIterationNum();
                ...
                if (flags[modelID]) {
                    ...
                    for (int m = 0; m < dim; m++) {
                        IMR imr = new IMR(unis.get(m), p, threshold0, maxIterations);
                        result = imr.run();
        ...
        MyFileUtils.exportResult(fileName, modelNames, ps, totalRMSE);
        fileName = basePath + "TIME.csv";
        MyFileUtils.exportResult(fileName, modelNames, ps, totalTime);
        fileName = basePath + "iterationNum.csv";
        MyFileUtils.exportResult(fileName, modelNames, ps, totalIterNum);
    }
}
