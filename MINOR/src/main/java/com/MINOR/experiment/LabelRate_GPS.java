    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < caseCnt; i++) {
            for (int seed = 0; seed < repeatTime; seed++) {
                ts = new TimeSeries();
                dataSetName = "GPS/GPS_lr_" + lrs[i] + '_' + seed;
        ...
        MyFileUtils.exportResult(fileName, modelNames, lrs, totalRMSE);
        fileName = basePath + "TIME.csv";
        MyFileUtils.exportResult(fileName, modelNames, lrs, totalTime);
        fileName = basePath + "iterationNum.csv";
        MyFileUtils.exportResult(fileName, modelNames, lrs, totalIterNum);
    }
}
