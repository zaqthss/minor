...
/**
 * Varying error length on ECG
 */
public class ErrorLength_ECG {
    public static void main(String[] args) {
        // experiment setting
        int dim = 32;
        String variableName = "el";
        DataSetType dt = DataSetType.ECG;
        ErrorInjectorType et = ErrorInjectorType.SHIFT;
        int p0 = Constants.p0;
        double threshold0 = Constants.thresholdECG0;
        int maxIterations = Constants.maxIteration0;
        int[] els = Constants.els;
        int caseCnt = els.length;
        int repeatTime = Constants.seeds.length;
        double S = 5.18285;      // speed constraint of MTCSC-C
        ...
        for (int i = 0; i < caseCnt; i++) {
            for (int seed = 0; seed < repeatTime; seed++) {
                ts = new TimeSeries();
                dataSetName = dt + "_" + et + "_" + variableName + "_" + els[i] + '_' + seed + ".csv";
        ...
        MyFileUtils.exportResult(fileName, modelNames, els, totalRMSE);
        fileName = basePath + "TIME.csv";
        MyFileUtils.exportResult(fileName, modelNames, els, totalTime);
        fileName = basePath + "iterationNum.csv";
        MyFileUtils.exportResult(fileName, modelNames, els, totalIterNum);
    }
}
